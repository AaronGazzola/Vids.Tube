...
- [ ] 1.1 `app/(app)/live/demo.types.ts`: add `mobileChrome: boolean` to
  `DemoLayoutConfig`, default `false` in `DEFAULT_DEMO_LAYOUT`, back-fill in
  `mergeDemoLayout`
- [ ] 1.2 `app/(app)/live/demo.stores.ts`: add `setMobileChrome(v: boolean)` to the
  layout store, updating `config.mobileChrome`
- [ ] 1.3 `app/(app)/live/page.tsx`: change the `useDemoLayout(...)` call to
...
- [ ] 2.1 `components/mobile-chrome.tsx`: reference constants object (all dimensions
...
- [ ] 2.2 Same file: `MobileChromeTopBar({ scale, handle, avatarUrl })` — back arrow,
  ...
  `scale`
- [ ] 2.3 Same file: `MobileChromeOverlay({ scale })` — chat rows with avatars, member
...
- [ ] 3.1 `components/live-player.tsx`: wrap the `<video>` in a shrink-wrapped
  ...
  video wrapper
- [ ] 3.2 `app/(app)/live/page.tsx`: floating Mobile layout chip (label + shadcn
...
- [ ] 4.1 `app/(app)/live/demo-preview.tsx`: measure the stage with a
...
- [ ] 4.2 Same file: add a "Mobile layout" `Switch` row to the overlay control panel
  (below the Goals reached row), bound to `config.mobileChrome`/`setMobileChrome`

## 5. Verify

- [ ] 5.1 `npx tsc --noEmit`, `npm run lint`, `doppler run -- npm run build` clean
- [ ] 5.2 `npx openspec validate add-mobile-layout-preview --strict`
- [ ] 5.3 Visual check against the reference screenshot: chrome on the demo stage at
