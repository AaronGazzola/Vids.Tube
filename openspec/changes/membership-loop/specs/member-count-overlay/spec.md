...
Along its left it SHALL read "Chat to become a Vids.Tube member", on a single line that never wraps. Down its right it SHALL stack the member total above the label "Members".
...
- **THEN** its left reads "Chat to become a Vids.Tube member" on one unbroken line
