...
- [ ] 1.1 New migration: create `public.credit_entries` with `id uuid primary key
...
- [ ] 1.2 In the same migration, enable row level security on `credit_entries`,
  grant select to `anon` and `authenticated`, and revoke insert/update/delete
  from both, matching the pattern used by `memberships`.
- [ ] 1.3 Add `public.credits_for_xp(xp bigint) returns bigint`, `language sql`,
  `immutable`, returning `greatest(floor(xp / 10), 0)::bigint`. The divisor lives
  only here.
- [ ] 1.4 Add `public.membership_credit_balance(p_membership_id uuid) returns
...
- [ ] 2.1 In the same migration, replace `public.recompute_membership` so that
...
- [ ] 2.2 Confirm by reading the replaced function that it never touches a row
  whose `kind` is not `earned`, so spending lines cannot be altered by a
  recompute.
- [ ] 2.3 Replace `public.merge_youtube_identity` so that, where it currently
...
- [ ] 2.4 Add `public.spend_credits(p_membership_id uuid, p_amount bigint, p_kind
...
- [ ] 2.5 Push the migration with `npx supabase db push` and regenerate
  `supabase/types.ts`.

## 3. Database: the enrichment setting

- [ ] 3.1 In the same migration, add `channels.chatter_enrichment_mode text not
...
- [ ] 3.2 Add a partial index on `channels (awaiting_enrichment)` where
...
- [ ] 4.1 Create `lib/channel-handle.ts` exporting `normalizeHandleBase(source:
...
- [ ] 4.2 Change `scripts/create-unclaimed-channels.ts` to import both from
  `lib/channel-handle.ts` and delete its local copies.
- [ ] 4.3 Add `tests/unit/channel-handle.test.ts` asserting: a display name with
...
- [ ] 5.1 Add `worker/lib/chatter-onboarding.ts` exporting
...
- [ ] 5.2 In that module, `full` mode SHALL call the existing YouTube
  ...
  false.
- [ ] 5.3 In that module, `deferred` mode SHALL create the channel from
  `authorName` and the message's `avatarUrl` with no external call, and SHALL set
  `awaiting_enrichment` to true.
- [ ] 5.4 In that module, a thrown or failed lookup in `full` mode SHALL be
...
- [ ] 5.5 After creating a channel, call `recompute_membership(channelId,
  communityId)` so the membership exists before the message is scored.
- [ ] 5.6 Add `resolveEnrichmentMode(channel)` to `worker/lib/streams.ts`
  returning the community channel's `chatter_enrichment_mode`, defaulting to
  `full` when absent.

## 6. Worker: wiring onboarding into ingest

- [ ] 6.1 In `worker/jobs/score.ts` `consumeYoutube`, resolve the enrichment mode
  once before the poll loop.
- [ ] 6.2 For each stored message, after the existing bot and host `continue`
  guards and before `ytBuffer.push`, call `ensureChatterChannel` for the author.
  Neither the host nor a bot SHALL reach this call.
- [ ] 6.3 Hold a `Set` of author ids already onboarded during this broadcast and
  skip the call for anyone in it, so onboarding costs one attempt per account per
  broadcast.
- [ ] 6.4 Do the same for site-origin messages, keyed on the sender's user id
  rather than a YouTube account, creating no channel when the sender already owns
  one.

## 7. Worker: refreshing memberships each batch

- [ ] 7.1 In `worker/jobs/score.ts`, after the block that writes `viewer_scores`
...
- [ ] 7.2 Log a `console.error` and continue when a single recompute fails, so
  one bad participant cannot stop the batch.
- [ ] 7.3 Skip the host and any bot participant, which the scoring buffer already
  excludes, and assert that by tracing the participant list.

## 8. Settings UI

- [ ] 8.1 Add `chatterEnrichment: boolean` to the settings tab's form state in
...
- [ ] 8.2 Extend the channel-settings save path in
...
- [ ] 8.3 Load the current value into the settings tab so the switch reflects the
  stored mode on mount.

## 9. Post-broadcast enrichment pass

- [ ] 9.1 Create `scripts/enrich-chatter-channels.ts` that selects channels where
...
- [ ] 9.2 Leave `awaiting_enrichment` true for any channel whose lookup returned
  nothing, and print how many were left for a later run.
- [ ] 9.3 Add the script to `package.json` as `enrich:chatters`.

## 10. Backfill and verification

- [ ] 10.1 Run `scripts/recompute-memberships.ts --apply` against production so
  every existing membership gains its earning line and cached balance, and record
  the before/after counts.
- [ ] 10.2 Create `scripts/verify-credit-ledger.ts` asserting, against
...
- [ ] 10.4 Add `tests/unit/credit-rate.test.ts` covering the earning rate as a
  pure function: 0 XP gives 0, 9 XP gives 0, 10 XP gives 1, 1640 XP gives 164,
  negative XP gives 0.
- [ ] 10.5 Run `npx tsc --noEmit`, `npm run lint`, and `npx vitest run`, and
  confirm all pass.
