...
The system SHALL update the channel's `streams` row when MediaMTX reports stream
readiness, via ingest routes guarded by a shared-secret header.

#### Scenario: Stream goes live

- **WHEN** MediaMTX posts the ready hook with the valid shared secret
- **THEN** the system sets the stream `status` to `live`, records `started_at` and
  `hls_path`, and stamps `last_seen_at`

#### Scenario: Stream ends

- **WHEN** MediaMTX posts the not-ready hook with the valid shared secret
- **THEN** the system sets the stream `status` to `ended` and records `ended_at`
