...
- [x] 1.1 Add `lib/step-result.ts` exporting `formatStepResult(obj)` producing a final
...
- [x] 1.2 Print a result line from each step script: `scripts/backfill-youtube-chat.ts`
...
- [x] 1.3 Add a `--video <id>` flag to `scripts/backfill-youtube-chat.ts` restricting the run to
...
- [x] 2.1 In `lib/post-broadcast-plan.ts`, widen `StepOutcome` to carry `result: unknown` and
      `status: "ok" | "failed" | "unknown"` in place of the boolean `ok`, keeping `error`.
- [x] 2.2 Add `judgeStep(step, exitedCleanly, result)` returning that status, with the per-step
...
- [x] 2.3 Change `isClean(outcomes)` to require every outcome be `ok`, so `unknown` prevents a
      clean record. Update `blockedBy` to treat anything other than `ok` as a blocker.
- [x] 2.4 In `worker/lib/post-broadcast.ts`, parse the result line from each step's output and
      record `judgeStep`'s verdict, storing the parsed result on the record in place of the
      current last-line-of-output detail.

## 3. Two phases

- [x] 3.1 In `lib/post-broadcast-plan.ts`, add `export type Phase = "score" | "settle"` and
...
- [x] 3.2 Add `phaseOwed(record, endedAt, nowMs)` returning `"score"` when no record is clean,
      `"settle"` when clean but not settled and at least 20 hours have passed since the broadcast
      ended, and `null` otherwise.
- [x] 3.3 Add `settleOutcome(archived, endedAt, nowMs)` returning `"merged"` when the fetch found
      messages, `"expired"` when it found none and 7 days have passed, and `"retry"` when it found
      none and less than 7 days have passed.
- [x] 3.4 Change `runPostBroadcastPass(streamId)` to `runPostBroadcastPass(streamId, phase)`,
...
- [x] 4.1 Create a migration adding `settled boolean not null default false`, `settled_at
...
- [x] 4.2 Backfill `settled = true` with `settle_note = 'repaired before phases existed'` for
...
- [x] 4.3 Check `supabase/migrations` against the remote with `supabase migration list --linked`
      before pushing, since `db push` applies every pending migration. Then push and regenerate
      `supabase/types.ts`.
- [x] 4.4 Replace `outstandingBroadcasts` in `worker/lib/post-broadcast.ts` with a query selecting
...
- [x] 5.1 Add `worker/maintain.ts`: run the preflight, take the broadcasts owing work, settle at
...
- [x] 5.2 Add the preflight to `worker/maintain.ts` in the shape of `worker/doctor.ts`: check
      ...
      completion record.
- [x] 5.3 Add `"maintain": "doppler run -- tsx worker/maintain.ts"` to `package.json`.
- [x] 5.4 Remove the post-broadcast block from the `finally` in `worker/index.ts`, leaving the
      lock release, and update the comment in `main()` that currently points at `npm run repair`.
- [x] 5.5 Update `scripts/repair-broadcasts.ts` to take `--phase score|settle`, defaulting to
...
- [x] 6.1 Add `scripts/macos/dev.vidstube.maintain.plist` running `npm run maintain` every 1800
      seconds with `RunAtLoad`, writing stdout and stderr to files under `~/Library/Logs/`.
- [x] 6.2 Add `docs/runbooks/maintenance-runner.md` covering what the machine needs (node,
...
- [x] 6.3 Record in that runbook that the live worker no longer settles broadcasts, so until the
      plist is loaded nothing settles them and `npm run maintain` must be run by hand.

## 7. Prove it

- [x] 7.1 Extend `tests/unit/post-broadcast-plan.test.ts` for `phaseOwed` across: no record;
...
- [x] 7.2 Add `tests/unit/step-result.test.ts` for `parseStepResult` over output with one result
...
- [x] 7.4 Run `npm run maintain` once against production and confirm it reports nothing owing,
      every broadcast having been settled by the backfill in 4.2.

## 8. Land it

- [x] 8.1 Run `openspec validate --strict` and archive the change.
- [x] 8.2 Add to `docs/runbooks/next-broadcast-checklist.md` that the first broadcast after this
