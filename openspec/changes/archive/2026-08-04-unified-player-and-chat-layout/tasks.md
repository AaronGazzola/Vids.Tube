...
- [x] 1.1 Add `components/video-player/types.ts` exporting the `PlayerSource`
...
- [x] 1.2 Create `components/video-player/use-media-source.ts`: a hook taking
...
- [x] 1.3 Remove the static `import Hls from "hls.js"` path from the codebase so hls.js
  is only reachable through the dynamic import in `use-media-source.ts`.
- [x] 1.4 Extend `components/video-player/use-video-state.ts` with `buffering` (true
...
- [x] 1.5 Add `atLiveEdge` and `secondsBehindLive` to the state hook, derived each sync
...
- [x] 2.1 Refactor `components/video-player/controls.tsx` to accept `transport`, `left`
  and `right` slot nodes and render the gradient/autohide shell around them, keeping
  the current visual treatment.
- [x] 2.2 Create `components/video-player/transport-vod.tsx` wrapping the existing
  `SeekBar` plus the elapsed/duration text and its `formatTime` helper.
- [x] 2.3 Create `components/video-player/settings-menu.tsx`: one gear menu that renders
  ...
  `0.5x`–`2x` options.
- [x] 2.4 Rewire `VideoPlayer.tsx` to consume `use-media-source` and the slot-based
...
- [x] 2.5 Add `buffering` and `error` presentation to the player: a spinner while
  buffering and an error state replacing the video when the source fails.

## 3. Fix the crop path in shared sizing

- [x] 3.1 In `VideoPlayer.tsx`, replace the forced `aspect-video` + `object-cover`
...
- [x] 3.2 Verify with a non-16:9 landscape VOD (4:3 or ultrawide) that the full frame is
...
- [x] 4.1 Create `components/watch-layout.tsx` exporting `WatchLayoutProvider`,
...
- [x] 4.2 Move fullscreen control out of `VideoPlayer.tsx`'s own container: the
...
- [x] 4.3 Wrap `app/watch/[videoId]/page.tsx` in `WatchLayoutProvider` + `WatchStage`
  with the player and the `ChatReplay` panel as its children, replacing the current
  ad-hoc grid wrapper.
- [x] 4.4 Wrap the live branch of `components/live-stream-view.tsx` in
...
- [x] 5.1 Create `components/video-player/transport-live.tsx`: a live indicator reading
  `atLiveEdge`, rendered active at the edge and inactive when behind, seeking to
  `liveSyncPosition` when activated while behind.
- [x] 5.2 Add an `overlay` (children) slot to `VideoPlayer.tsx`, rendered above the
  video and below the control bar.
- [x] 5.3 Add the quality section to `settings-menu.tsx` from the engine's `levels`,
  with an auto entry, calling `setLevel`; hide the section entirely for `mp4` sources.
- [x] 5.4 Fold the muted-autoplay state into the shared volume control: live sources
  start muted, and a prominent unmute affordance shows while muted and dismisses on
  unmute. Reuse the existing pill treatment.
- [x] 5.5 Rewrite `components/live-player.tsx` as a thin wrapper that renders
...
- [x] 5.6 Move `DisconnectedOverlay` and the viewer-cap states in
  `components/live-stage.tsx` into the overlay slot rather than sibling absolute
  positioning.
- [x] 5.7 Gate seek behaviour by source: arrow-key seeking and the seek bar are absent
  for live sources; `Space`/`k`, `f` and `m` work identically on both.

## 6. Bounded chat containers

- [x] 6.1 Remove `min-h-80` from `components/live-chat.tsx` and
...
- [x] 6.2 In `components/live-stream-view.tsx`, replace the `lg:h-[70vh]` wrappers
...
- [x] 6.3 In `app/watch/[videoId]/page.tsx`, replace the
  `cn(replayExpanded && "lg:h-[70vh]")` wrapper (line 99) the same way, including the
  case where the replay is expanded below `lg`.
- [x] 6.4 Give `WatchStage` its single-column layout: a `100dvh` grid (with a `vh`
...
- [x] 6.5 Confirm `YoutubeVerifyBanner` in `live-chat.tsx` renders between the header
  and the message list, outside the scroll container, so it is never scrolled away.
- [x] 6.6 Verify sticky-scroll behaviour now that containers overflow: following at the
...
- [x] 7.1 Render the chat slot as a translucent overlay pinned to one side of
  `WatchStage` when `mode` is `fullscreen`, keeping the same React element so the chat
  is not remounted across the mode change.
- [x] 7.2 Add a chat-overlay visibility toggle to the control bar's right slot, shown
...
- [x] 8.1 Run `npx tsc --noEmit` and `npm run build`; confirm no type errors and that
  the watch-page bundle does not include hls.js.
- [x] 8.2 Verify on the live page and the watch page at 375px, 768px and 1440px that a
...
- [x] 8.3 Verify at desktop width that the chat column's bottom edge aligns with the
  player column's on both pages.
- [x] 8.6 Add a test asserting the chat message list is the scrolling element and the
  page body does not overflow at a narrow viewport, on both the live page and the watch
  page.
- [x] 8.7 Add a test asserting a non-16:9 landscape VOD renders without cropping, and
