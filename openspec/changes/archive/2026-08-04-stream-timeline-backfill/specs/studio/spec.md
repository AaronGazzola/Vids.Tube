...
**Reason**: The `/studio` overview page and its placeholder metric cards were retired
when the control room moved to `/live`; no such route exists in the code. `/studio` is
now the stream list, which is a real surface rather than placeholder cards.

**Migration**: Owner-facing stream status lives on `/live`. `/studio` lists the
channel's streams.

### Requirement: Upload tool (placeholder)

**Reason**: Retired with the rest of the placeholder Studio shell. Vids.Tube has no
upload path by design — all content derives from streams (AZ-189) — so this
placeholder describes a feature that will never be built.

**Migration**: None. There is no upload path.

### Requirement: Videos management (placeholder)

**Reason**: Retired with the placeholder Studio shell. Its stubbed rows are replaced
by the real stream list at `/studio`.

**Migration**: Use the stream list at `/studio`.

### Requirement: Channel settings (placeholder)

**Reason**: Retired with the placeholder Studio shell; channel details are edited from
the channel page and `/account`.

**Migration**: Edit channel details on the channel page and account settings.
