...
differences SHALL be shown as: a checkmark drawn into an empty box for a task that became completed, a
struck-through line for a completed task, a task appearing for one that was added, a task disappearing
for one that was removed, and a status change shown in place for any other move between statuses.
