...
- [ ] 4.2 (deferred — needs the binaries; tracked as **AZ-123**) `npm run worker:doctor` reports correct PASS/FAIL (verify by pointing a binary path at a missing file)
- [ ] 4.3 (deferred — needs a live stream + binaries; tracked as **AZ-123**) Run `npm run worker` against a live (or `ffmpeg`-replayed) HLS source with an `enabled` `chat_scoring_state` row; confirm `transcript_segments` rows appear with increasing `start_s` and a restart resumes after `max(end_s)` without duplicates
