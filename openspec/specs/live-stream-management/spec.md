# live-stream-management Specification

## Purpose

Give the owner one page — `/live` — that manages the channel's single active
broadcast end to end: a Settings tab saving through one action, a Preview tab with
the private/public player and live transcript, an Activity tab with goals,
competition, mod-bot actions and chat, and a fixed status toolbar driving the
draft/schedule/preview/live lifecycle.
