# mobile-layout-preview Specification

## Purpose

Let the owner overlay a representational YouTube-mobile chrome (top channel bar,
overlaid live chat, reaction button, chat input) on the /live preview and the demo
overlay stage, at true scale and position, so stream overlays can be placed around
the mobile UI instead of under it.
