...
One deterministic "!" command layer over both chat origins: a per-channel
registry with tunable cooldowns and per-stream totals, an auditable execution
log, a shared parser, the !help builtin, and the public per-channel command
guide page. Every later command feature builds on this capability.
